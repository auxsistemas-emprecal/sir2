...
export default function CuadreCaja() {
  ...
  const [arqueo, setArqueo] = useState(() => {
    ...
    return savedArqueo ? JSON.parse(savedArqueo) : {
      100000: 0, 50000: 0, 20000: 0, 10000: 0, 5000: 0, 2000: 0, monedas: 0,
    };
  });
  ...
  const handleFinalizarCuadre = async () => {
    ...
    const datosArqueo = {
      ...
      estado_caja:
        conciliacion.diferencia === 0
          ...
          ? "SOBRANTE"
          : "FALTANTE",
      usuario: usuarioActual, // ✅ Nuevo campo agregado
    };
    ...
    try {
      ...
      setArqueo({ 100000: 0, 50000: 0, 20000: 0, 10000: 0, 5000: 0, 2000: 0, monedas: 0 });
      setRecibosCaja(0);
    ...
    <div id="cuadre-print" className="max-w-6xl mx-auto p-4 md:p-8 bg-slate-50 min-h-screen font-sans text-slate-900">
      ...
      <div className="print-only-block hidden">
        <div className="flex justify-between items-center mb-4 border-b pb-2">
           ...
           <p className="text-xs italic">Generado por: {localStorage.getItem("usuario") || "Admin"}</p>
        </div>
        ...
        <table className="print-table">
          ...
          <tbody>
            ...
            <tr className="font-bold">
              <td colSpan="2" className="text-right">TOTAL GASTOS PAGADOS</td>
        ...
        <table className="print-table">
            <tbody>
                ...
                <tr><td>(+) Recibos de Caja</td><td className="text-right">${Number(recibosCaja).toLocaleString("es-CO")}</td></tr>
                ...
                <tr className="font-bold bg-slate-100"><td>TOTAL NETO DEL DÍA</td><td className="text-right">${resumenFinal.totalNetoGeneral.toLocaleString("es-CO")}</td></tr>
                <tr className="font-bold"><td>EFECTIVO ESPERADO EN CAJA</td><td className="text-right">${resumenFinal.totalEfectivoNeto.toLocaleString("es-CO")}</td></tr>
            </tbody>
        </table>

        <h3 className="text-[12px] font-bold mb-2 uppercase">🧾 Conteo Físico</h3>
        <table className="print-table">
        ...
        <div className="box-conciliacion-print">
          <p><strong>EFECTIVO ESPERADO:</strong> ${resumenFinal.totalEfectivoNeto.toLocaleString("es-CO")}</p>
          <p><strong>ESTADO DE CAJA:</strong> {conciliacion.mensaje}</p>
          <p style={{ fontSize: "16px", fontWeight: "bold" }}>DIFERENCIA: ${Math.abs(conciliacion.diferencia).toLocaleString("es-CO")}</p>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 space-y-8 animate-slide-up">
          <section className="bg-white rounded-3xl shadow-sm border border-slate-200 p-6">
            ...
            <form onSubmit={agregarGasto} className="no-print grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 bg-slate-50 p-4 rounded-2xl">
              ...
              <input name="observacion" placeholder="Nota / Observación" value={formGasto.observacion} onChange={handleGastoChange} className="p-2.5 rounded-xl border-slate-200" />
              ...
              <tbody>
                {gastosHoy.map((g) => (
                  <tr key={g.id} className="border-b border-slate-50 hover:bg-slate-50">
                    ...
                    <td className="py-3 text-slate-500 italic">{g.observacion || "-"}</td>
                    <td className="py-3 text-right font-bold">${g.valor.toLocaleString()}</td>
                    <td className="py-3 text-center">
                      <button onClick={() => eliminarGasto(g.id)} className="text-rose-400 hover:text-red-600">✕</button>
                    </td>
                  </tr>
                ))}
                <tr className="font-extrabold text-emerald-800 bg-emerald-50/50">
                  <td colSpan="2" className="py-4 px-4">TOTAL EGRESOS</td>
                  <td className="py-4 text-right">${totalGastosHoy.toLocaleString()}</td>
                  <td></td>
                </tr>
              </tbody>
            </table>
        ...
        <div className="space-y-8 animate-slide-up">
          <section className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
            ...
            <div className="p-6 space-y-4">
              ...
              <hr />
              <div className="flex justify-between items-end"><span className="text-[10px] font-black text-slate-400 uppercase">Esperado en Caja</span><span className="text-3xl font-black text-emerald-600">${resumenFinal.totalEfectivoNeto.toLocaleString()}</span></div>
            </div>
          </section>
          ...
          <button onClick={handleFinalizarCuadre} className="no-print w-full bg-slate-900 text-white py-5 rounded-3xl font-black text-lg shadow-xl active:scale-95 transition-all">
            💾 GUARDAR CIERRE
          </button>
        </div>
      </div>
            ...
            <tbody>
              {movimientosFiltrados.map((m, i) => (
                <tr key={i} className="border-b border-slate-50 hover:bg-slate-50">
                  ...
                  <td className="p-4 text-right font-black">${Number(m.total).toLocaleString()}</td>
                  <td className="p-4 text-center">
                    <span className="px-3 py-1 bg-white border rounded-full text-[10px] font-black">{obtenerTipoPago(m)}</span>
                  </td>
                </tr>
              ))}
            </tbody>
...
//           /* ESTILOS DE TABLA DE IMPRESIÓN */
//           .print-table { width: 100%; border-collapse: collapse; margin-bottom: 1rem; display: none; table-layout: fixed; }
//           .print-table th, .print-table td { border: 1px solid #000; padding: 6px 10px; font-size: 11px; word-wrap: break-word; }
//           .print-table th { background-color: #f1f5f9; font-weight: bold; text-align: left; }
          
//           /* NUEVO ORDEN DE ANCHOS SEGÚN TU DIBUJO */
//           .col-print-desc { width: 30%; }
//           .col-print-nota { width: 50%; }
//           .col-print-valor { width: 20%; }
...
//       {/* VISTA DE IMPRESIÓN (GASTOS) - ORDEN CORREGIDO SEGÚN DIBUJO */}
//       <div className="print-only-block hidden">
...
//         {/* ... Resto de tablas de impresión (Balance, Conteo) ... */}
//         <h3 className="text-[12px] font-bold mb-2 uppercase">📊 Balance de Sistema</h3>
//         <table className="print-table">
//             <tbody>
//                 <tr><td>(+) Ventas en Efectivo</td><td className="text-right">${totalesPorTipo.Efectivo.toLocaleString("es-CO")}</td></tr>
