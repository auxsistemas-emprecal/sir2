...
const RemisionesModal = ({
  isOpen,
  onClose,
  listaRemisiones,
  onVerRemisionIndividual,
}) => {
        ...
        <div className="bg-orange-600 p-6 text-white flex justify-between items-center">
          <div>
            ...
            <p className="text-orange-100 text-xs font-medium mt-1">
              Documentos asociados a este anticipo
            </p>
          </div>
          <button
            onClick={onClose}
            className="bg-white/20 hover:bg-white/30 p-2 rounded-full transition-colors cursor-pointer hover:rotate-90 duration-200"
          >
            <X size={20} />
          </button>
        </div>
...
export default function AnticiposArchived({
  data,
  toggleAnticipoEstado,
  onVerAnticipo,
  onVerDetalleRemision,
}) {
  ...
  // Solo necesitamos el conteo de vigentes para la sección de búsqueda
  const stats = {
    vigentes: data.filter((a) => a.estado === "VIGENTE").length,
  };
  ...
  const renderTipoPagoIcon = (tipo) => {
    ...
    if (t.includes("transferencia") || t.includes("banco"))
      return <Landmark size={12} className="text-blue-500" />;
    if (t.includes("nequi") || t.includes("daviplata"))
  ...
  const handleToggleEstado = (anticipo) => {
    ...
    setNotificationMessage(
      `Anticipo de ${anticipo.tercero} marcado como ${newState}.`
    );
    setTimeout(() => setNotificationMessage(null), 4000);
  };

  // --- FUNCIÓN DE ACTUALIZACIÓN DE PAGO ---
  const handlePagoClick = async (anticipo) => {
    try {
      // 1. Llamamos a la función que viene por props para actualizar en BD
      // Esta función debería recibir el anticipo y cambiar el valor de 'pagado' (0 a 1 o viceversa)
      // await onTogglePago(anticipo);

      // 2. Determinamos el nuevo estado para el mensaje (basado en el estado actual antes del cambio)
      let anticipoNuevoEstadoPayload = await fetchPagosPorNoIngreso(
        anticipo.noComprobante
      );
      anticipoNuevoEstadoPayload = anticipoNuevoEstadoPayload[0];
      anticipo.pagado = +!anticipo.pagado;
      anticipoNuevoEstadoPayload = {
        ...anticipoNuevoEstadoPayload,
        pagado: +!anticipoNuevoEstadoPayload.pagado,
      };
      const nuevoEstado = anticipo.pagado === 1 ? "PENDIENTE" : "PAGADO";
      console.log(anticipoNuevoEstadoPayload);
      await updatePago(
        anticipoNuevoEstadoPayload.no_ingreso,
        anticipoNuevoEstadoPayload
      );
      ...
      // 4. Limpiar notificación después de 4 segundos
      setTimeout(() => setNotificationMessage(null), 4000);
    } catch (error) {
      console.error("Error al actualizar el pago:", error);
      setNotificationMessage("Error al intentar actualizar el estado de pago.");
      setTimeout(() => setNotificationMessage(null), 4000);
    }
  };
  ...
  const renderBotonRemisionesUnico = (remisionesRaw) => {
    if (!remisionesRaw || remisionesRaw === "[]" || remisionesRaw === "") {
      return (
        <span className="text-gray-300 text-[10px] font-black uppercase tracking-tighter italic">
          Sin remisiones
        </span>
      );
    }
    ...
    if (lista.length === 0)
      return (
        <span className="text-gray-300 text-[10px] font-black tracking-tighter">
          Sin remisiones
        </span>
      );
    ...
    return (
      <div className="flex justify-end">
        ...
        >
          <Tags
            size={14}
            className="text-orange-400 group-hover/rem:text-white transition-colors"
          />
  ...
  return (
    <div className="bg-gray-50/50 rounded-3xl p-1 border border-gray-200 shadow-sm relative overflow-hidden font-sans">
      ...
      {/* CABECERA CON GRÁFICA DE FONDO */}
      <div className="relative bg-[#f8fafc] rounded-t-[22px] px-8 py-8 border-b border-gray-100 flex flex-col lg:flex-row lg:items-center justify-between gap-6 overflow-hidden">
        {/* Gráfica SVG de Profundidad */}
        <div className="absolute inset-0 pointer-events-none opacity-[0.12]">
          ...
          >
            <path
              d="M0,85 Q40,30 90,60 T180,40 T280,75 T400,15 L400,100 L0,100 Z"
              fill="url(#depthGradient)"
            />
            <path
              d="M0,85 Q40,30 90,60 T180,40 T280,75 T400,15"
              fill="none"
              stroke="#3b82f6"
              strokeWidth="1.2"
              strokeDasharray="4 2"
        ...
        <div className="group cursor-default relative z-10">
          <h2 className="text-2xl font-black text-gray-800 flex items-center gap-3">
            <div className="bg-blue-600 p-2.5 rounded-2xl text-white shadow-lg shadow-blue-200 group-hover:rotate-6 transition-transform duration-300">
              <Archive size={22} />
            </div>
            Historial de Anticipos
          </h2>
          <p className="text-gray-400 text-sm font-medium mt-1 ml-14">
            Trazabilidad de ingresos y saldos
          </p>
        </div>
        ...
        <div className="flex flex-col gap-3 relative z-10">
          <div className="relative group">
            <input
              ...
              className="w-full lg:w-96 pl-12 pr-4 py-3.5 bg-white/90 backdrop-blur-md border border-gray-200 rounded-2xl text-sm focus:ring-4 focus:ring-blue-500/10 focus:bg-white focus:border-blue-300 transition-all font-semibold outline-none shadow-sm"
              value={searchTerm}
          ...
          {/* KPI SOLAMENTE DE VIGENTES */}
          <div className="flex items-center gap-2 px-2">
            <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse shadow-[0_0_8px_rgba(16,185,129,0.6)]"></div>
            <span className="text-[10px] font-black text-gray-500 uppercase tracking-widest">
              {stats.vigentes} Anticipos Vigentes
            </span>
          </div>
        </div>
      </div>
        ...
        <table className="min-w-full border-separate border-spacing-0">
          <thead>
            <tr className="bg-gray-50/50">
              ...
              <th className="px-4 py-4 text-right text-[10px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-100">
                Valor
              </th>
              <th className="px-4 py-4 text-right text-[10px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-100">
                Vínculos
              ...
              >
                ...
                <td className="px-6 py-5">
                  <div className="flex flex-col group/doc">
                    <span className="text-blue-600 font-black text-sm group-hover/doc:translate-x-1 transition-transform cursor-default">
                      #{anticipo.noComprobante}
                    </span>
                ...
                <td className="px-6 py-5">
                  <div className="flex items-center gap-3 group/avatar">
                    <div
                      className="w-9 h-9 rounded-full bg-linear-to-br from-gray-100 to-gray-200 
                ...
                <td className="px-6 py-5">
                  <div className="flex flex-col">
                    <span className="text-gray-600 text-[11px] font-medium truncate max-w-[130px] italic">
                      "{anticipo.concepto}"
                    </span>
                ...
                <td className="px-6 py-5">
                  <div className="flex flex-col items-center justify-center gap-2">
                    <button
                      onClick={() => handlePagoClick(anticipo)}
                      className={`px-5 py-2 rounded-full border-2 font-black text-[11px] shadow-sm flex items-center gap-2 transition-all duration-300 active:scale-90 cursor-pointer group/pill ${
                        anticipo.pagado === 1
                          ? "bg-[#E6F9F4] border-[#81E3C9] text-[#008A6B] shadow-[#81E3C9]/10 hover:bg-[#008A6B] hover:text-white"
                          : "bg-[#FFF0F3] border-[#FFC1CC] text-[#D10043] shadow-[#FFC1CC]/10 hover:bg-[#D10043] hover:text-white"
                      }`}
                    >
                      <span
                        className={`w-1.5 h-1.5 rounded-full transition-colors ${
                          anticipo.pagado === 1
                            ? "bg-[#008A6B] group-hover/pill:bg-white"
                            : "bg-[#D10043] group-hover/pill:bg-white"
                        }`}
                      ></span>
                      {anticipo.pagado === 1 ? "PAGADO" : "PENDIENTE"}
                    </button>
                    {anticipo.fechaPagado && (
                      <span className="text-[9px] text-gray-400 font-bold uppercase tracking-tighter opacity-70">
                        {new Date(anticipo.fechaPagado).toLocaleDateString(
                          "es-CO"
                        )}
                      </span>
                    )}
                ...
                <td className="px-4 py-5 text-right">
                  <span
                    className={`text-sm font-black px-2.5 py-1.5 rounded-xl transition-all ${
                      (anticipo.saldo || 0) > 0
      ...
      <div className="bg-white px-8 py-5 text-[11px] text-gray-400 font-bold flex justify-between items-center rounded-b-3xl border-t border-gray-100">
        <div className="flex gap-4 items-center">
          ...
          <div className="flex items-center gap-2 bg-gray-50 px-3 py-1.5 rounded-full border border-gray-100">
            <Calculator size={14} className="text-emerald-500" /> Totales
            Actualizados
          </div>
        </div>
        <div className="text-gray-300 tracking-widest uppercase flex items-center gap-2 hover:text-blue-400 transition-colors cursor-default">
          SISTEMA GESTIÓN <ChevronRight size={14} />{" "}
