...
// ✅ Ruta corregida para resolver el error "Failed to resolve import"
...
export default function Inicio({
  ...
  // --- EFECTO REACTIVO: CONSULTA AL CAMBIAR FECHAS ---
  useEffect(() => {
    const cargarGraficas = async () => {
      ...
      setLoading(true);
      try {
        // La función fetchVentasPorFecha ya incluye los headers de autorización
  ...
  const stats = useMemo(() => {
    ...
    const totalVentasHistoricas = activeMovements.reduce((acc, m) => acc + (Number(m.total) || 0), 0);
    ...
    return {
      ventasHistoricas: totalVentasHistoricas,
      ventasHoy,
      ...
      pieData: [
        { name: "Pagados", value: activeMovements.filter(m => m.pagado === 1).length, color: "#6366f1" },
        { name: "Pendientes", value: activeMovements.filter(m => m.pagado === 0).length, color: "#f43f5e" },
      ],
    };
  }, [datosApi, movements, anticipos, terceros, hoy]);
    ...
    <div className="w-full max-w-[1600px] mx-auto p-4 space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="relative overflow-hidden rounded-[24px] md:rounded-[32px] bg-slate-900 text-white p-6 md:p-10 shadow-2xl">
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6 md:gap-8">
            ...
            <div className="flex flex-wrap items-center gap-4 mt-4 bg-white/5 p-3 rounded-2xl border border-white/10 w-fit">
              <div className="flex items-center gap-2">
                ...
                <input 
                  ...
                  className="bg-transparent text-xs font-bold focus:outline-none cursor-pointer hover:text-indigo-300 transition-colors text-white"
                />
              </div>
          ...
          <button
            ...
            className="flex items-center justify-center gap-3 bg-indigo-600 hover:bg-indigo-500 text-white px-6 py-4 md:px-8 md:py-4 rounded-2xl font-bold transition-all shadow-xl shadow-indigo-500/20 w-full lg:w-auto"
          >
            <Package size={20} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="md:col-span-2 lg:col-span-3 grid grid-cols-1 sm:grid-cols-3 gap-6">
          <StatCard icon={<TrendingUp size={24} />} label="Ventas Totales" value={`$${stats.ventasHistoricas.toLocaleString()}`} color="indigo" onClick={() => setTab && setTab("movimientos")} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="pro-card p-6">
          ...
          <div className="mb-4">
            ...
            <p className="text-[10px] text-slate-400 uppercase font-bold tracking-widest">
              Conteo de despachos realizados
            </p>
          </div>
          <div className="h-[300px]">
            <Plot
              data={[{
                ...
                hovertemplate: "%{y} remisiones<extra></extra>"
              }]}
              layout={{
                autosize: true,
      ...
      {/* SECCIÓN DE ÚLTIMOS MOVIMIENTOS Y CARTERA */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="pro-card p-6 md:p-8 lg:col-span-2 overflow-hidden">
          <div className="flex justify-between items-center mb-8">
            ...
            <button onClick={() => setTab && setTab("movimientos")} className="text-sm font-bold text-indigo-600 flex items-center gap-1">
...
//     // Ventas de hoy (Para el velocímetro)
//     const totalHoy = activeMovements
//       .filter(m => m.fecha?.split("T")[0] === hoyISO)
//       .reduce((acc, m) => acc + (Number(m["Total Neto Día"]) || Number(m.total) || 0), 0);

//     // Valor unificado histórico
...
//     // META VARIABLE basada en el desempeño de hoy
//     const metaBase = 5000000;
//     const metaAjustada = totalHoy > metaBase ? totalHoy * 1.2 : metaBase;

//     const ventasPorDia = activeMovements.reduce((acc, m) => {
//       const fecha = m.fecha?.split("T")[0] || "Sin fecha";
...
//               Estado de operación: Tienes{" "}
//               <span className="text-white font-bold">
//                 {movements.filter((m) => m.estado !== "ANULADA").length} remisiones
//               </span>{" "}
//               activas.
//             </p>
//           </div>
...
//       {/* KPIs CON VELOCÍMETRO ACTUALIZADO */}
//       <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
//         <div className="md:col-span-2 lg:col-span-3 grid grid-cols-1 sm:grid-cols-3 gap-6">
//           <StatCard
...
//                     <tr key={idx} className="group hover:bg-slate-50/50 transition-colors">
//                       <td className="py-4 flex items-center gap-3">
//                         <div className={`avatar-circle flex-shrink-0 ${avatar.color}`}>
//                           {avatar.initials}
//                         </div>
//                         <span className="font-bold text-slate-700 text-sm capitalize truncate max-w-[120px] md:max-w-none">
//                           {m.tercero?.toLowerCase()}
//                         </span>
//                       </td>
//                       <td className="py-4 text-sm font-semibold text-slate-500">REM-{m.remision}</td>
...
//       <div className={`h-14 w-14 rounded-2xl flex items-center justify-center ${themes[color]} border mb-4 shadow-sm`}>
//         {icon}
//       </div>
//       <div>
//         <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">{label}</p>
//         <div className="flex items-end justify-between">
//           <p className="text-xl md:text-2xl font-black text-slate-900 tabular-nums truncate">{value}</p>
//           <div className={`p-1.5 rounded-lg ${themes[color]} bg-opacity-20 flex-shrink-0`}>
//             <ArrowUpRight size={16} />
//           </div>
//         </div>
//       </div>
...
//     const metaBase = 5000000;
//     const metaAjustada =
//       valorUnificado > metaBase ? valorUnificado * 1.2 : metaBase;
...
//               Estado de operación: Tienes{" "}
//               <span className="text-white font-bold">
//                 {movements.filter((m) => m.estado !== "ANULADA").length}{" "}
//                 remisiones
//               </span>{" "}
//               activas.
//             </p>
...
//                     steps: [
//                       {
//                         range: [0, stats.metaAjustada * 0.7],
//                         color: "#f1f5f9",
//                       },
//                       {
//                         range: [stats.metaAjustada * 0.7, stats.metaAjustada],
//                         color: "#e0e7ff",
//                       },
//                     ],
//                   },
...
//             Desglose de Ingresos, Despachos y Volumen
//           </p>
//         </div>
//         <div className="h-[300px] md:h-[400px]">
//           <Plot
...
//                       <td className="py-4 flex items-center gap-3">
//                         <div
//                           className={`avatar-circle flex-shrink-0 ${avatar.color}`}
//                         >
//                           {avatar.initials}
//                         </div>
//                         <span className="font-bold text-slate-700 text-sm capitalize truncate max-w-[120px] md:max-w-none">
...
//                       <td className="py-4 text-right">
//                         <span
//                           className={`px-3 py-1 rounded-full text-[10px] font-black inline-block ${
//                             m.estado === "ANULADA"
...
//     <div
//       onClick={onClick}
//       className="pro-card p-6 flex flex-col justify-between min-h-[160px] cursor-pointer"
//     >
//       <div
//         className={`h-14 w-14 rounded-2xl flex items-center justify-center ${themes[color]} border mb-4 shadow-sm`}
//       >
//         {icon}
//       </div>
//       <div>
...
//           </p>
//           <div
//             className={`p-1.5 rounded-lg ${themes[color]} bg-opacity-20 flex-shrink-0`}
//           >
//             <ArrowUpRight size={16} />
//           </div>
//         </div>
